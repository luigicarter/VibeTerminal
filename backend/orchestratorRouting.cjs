...
function sessionNativeIdentity(session) {
  ...
  return { provider: session.provider || session.kind, home: session.providerProfileId ? `custom:${session.providerProfileId}` : session.home || (session.openFusion || session.kind === 'openfusion' ? 'openfusion' : 'global'), workspace: session.cwd, id: session.conversationId || native?.id };
}
