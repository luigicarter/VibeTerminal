...
Finish with exactly one choose call. For reuse provide targetId and an existing workItemId when continuing it. For create provide kindOfSession; include workItemId only for a fresh authorized handoff of that existing task. reason is one short factual explanation of the resource choice, not a claim that work was delivered or completed.`;
...
async function planTaskRoute({ context, complete, read, check = () => {}, maxRounds = 8, resetReadBudget = () => {} }) {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (choice?.finish_reason === 'length') throw new Error('Routing interpretation was incomplete; no terminal was assigned.');
