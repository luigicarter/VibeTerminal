...
function commandFields(kind) {
  ...
  if (kind === 'operate_terminal') ['promptMode', 'answerMode', 'permissionMode', 'answerText', 'answerTexts'].forEach(key => allowed.add(key));
...
When clarification is necessary, ask one brief, natural question directly. Do not repeat the user request or narrate the interpretation process.
