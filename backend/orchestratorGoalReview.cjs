...
const { completionOptions, outputTokensFor } = require('./orchestratorModelOptions.cjs');
...
function createGoalReviewer({ complete, recordDiagnostic, redact = value => value, now = Date.now }) {
  ...
  return {
    async inspect({ owner, goal, evidence, model, signal, target, diagnosticContext = {} }) {
      ...
      const response = await complete({ model: model.id, max_tokens: tokens, ...completionOptions(model),
...
module.exports = { createGoalReviewer, INSPECTION_GOAL_REVIEW };
