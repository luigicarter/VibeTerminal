...
assert(
  terminalPaneSource.includes('["codex", "open-codex"].includes(sessionRef.current.kind)') &&
...
assert(
  appSource.includes('!session || !["codex", "open-codex"].includes(session.kind) || session.status !== "running"') &&
    appSource.indexOf('!session || !["codex", "open-codex"].includes(session.kind) || session.status !== "running"') <
...
assert(
  appSource.includes('["codex", "open-codex"].includes(session.kind) && status === "waiting"') &&
...
assert(
  fusionChatPaneSource.includes('normalized === "/clear"') &&
    ...
    fusionChatPaneSource.includes('<FusionCommandPalette') &&
