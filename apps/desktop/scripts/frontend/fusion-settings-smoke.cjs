...
// Empty/disabled asynchronous menus never select a placeholder or produce NaN.
{
  const { moveSlashIndex, searchSlashCommands } = menuModule.exports;
  const rows = [{ key: "loading", label: "Loading", desc: "" },
    { key: "first", label: "First", desc: "", command: "/first" },
    { key: "disabled", label: "Disabled", desc: "", command: "/bad", disabled: true },
    { key: "last", label: "Last", desc: "", command: "/last" }];
  assert(moveSlashIndex([], 0, "ArrowDown") === -1, "empty navigation has no selection");
  assert(moveSlashIndex(rows, 1, "ArrowDown") === 3, "navigation skips disabled rows");
  assert(moveSlashIndex(rows, 3, "ArrowDown") === 1, "navigation wraps to a selectable row");
  assert(moveSlashIndex(rows, 1, "End") === 3 && moveSlashIndex(rows, 3, "Home") === 1, "Home/End respect selectable rows");
  assert(searchSlashCommands(FUSION_SLASH_COMMANDS, "/planner")[0].name === "/planner", "exact matches outrank description matches");
  assert(buildFusionPicker({ role: "executor" }, "gpt-6-astra").items.some(item => item.command === "__model:codex/gpt-6-astra"), "search finds a model before choosing a family");
  assert(codexEffortValuesForModel("gpt-6-astra").includes("max"), "Astra offers supported reasoning levels");
  assert(codexEffortValuesForModel("auto", { codex: [{ id: "runtime-default", label: "Default", isDefault: true, supportedEfforts: ["low", "ultra"] }] }).includes("ultra"), "Auto follows the live default's effort capabilities");
  assert(buildSlashMenu("/effort execution", { executorEffort: "high" }).items[0]?.current, "current reasoning effort leads the picker");
}
...
}
{
  ...
  assert(["/planner", "/executor", "/planner-model", "/executor-model", "/speed", "/effort", "/fast", "/refresh-models"].every(name => visibleCommands.includes(name)),
    "model and reasoning controls must be discoverable at the top level");
  const menu = buildSlashMenu("/");
  assert(menu.items.every(item => item.section && (item.command || item.fill)), "every command has a group and an action");
  assert(buildSlashMenu("/reasoning").items.some(item => item.fill === "/effort "), "search includes descriptions");
  assert(buildSlashMenu("/sessions").items[0]?.command === "/resume", "search supports aliases");
  assert(buildSlashMenu("/fast ").items.every(item => item.command.startsWith("/fast ")), "fast serving opens an actionable submenu");
}
...
// ---- role settings submenus: model + effort + fast per role ----
{
  ...
  assert(
    menu.title === "Planner (Claude)" &&
      model?.label === "Model — Sonnet · latest" &&
...
);
{
  ...
  assert(
    menu.items.some(item => item.command === "/claude sonnet") && menu.items.some(item => item.command === "/claude claude-sonnet-5"),
...
}
{
  ...
  assert(
    menu.items[0]?.command === "/claude opus" &&
      ...
      menu.items.filter(item => item.command === "/claude claude-sonnet-5").length === 1 &&
  ...
  assert(
    picker.items.find((item) => item.command === "__family:claude")?.desc.includes("6 models"),
...
assert(
  !pane.includes('applySpeedPreset("harness", "fast")') &&
    ...
    pane.includes("FUSION_SLASH_COMMANDS.map"),
...
assert(
  fs.readFileSync(path.join(rootDir, "frontend/components/FusionCommandPalette.tsx"), "utf8").includes("onMouseMove=") &&
    !fs.readFileSync(path.join(rootDir, "frontend/components/FusionCommandPalette.tsx"), "utf8").includes("onMouseEnter="),
