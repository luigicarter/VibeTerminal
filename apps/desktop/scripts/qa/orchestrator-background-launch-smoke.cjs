...
process.on('uncaughtException',error=>{console.error(error.stack||String(error));process.exit(1);});
if(process.env.VIBE_QA_TRACE_OBSERVATION==='1'){
 const observation=require(${JSON.stringify(path.join(root,'backend/terminalObservation.cjs'))}),create=observation.createTerminalObservation;
