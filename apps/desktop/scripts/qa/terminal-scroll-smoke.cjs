...
const modules = ['terminalOutput', 'terminalWheel', 'terminalScrollback', 'terminalImages'].map(name => {
...
window.makeFixture = async () => {
  ...
  const terminalExitedRef={current:false},createdRef={current:true},session={id:'fixture',kind:'terminal'},sessionRef={current:session},runtimeScope=()=>({});
