...
(async () => { try {
  ...
  const sent = toolsFrom(await command(`Send ${session.handle}: ${payload}`)); assert.equal(sent[0].ok, true, JSON.stringify(sent)); assert.equal(sent[0].status, "written");
  ...
  const refused = toolsFrom(await command(`Send ${session.handle}: Write-Output legitimate`), false); assert.equal(refused[0].ok, false); assert.match(refused[0].error, /COMPLETE|payload|authorized user task/i);
  ...
  await cdp.eval(`window.__pendingRelay=window.vibe.orchestrator.send({text:'Read ${session.handle}',origin:'text'});void 0`);
  await until(() => fs.existsSync(path.join(output, "held")), "held inference");
  const busy = await cdp.eval(`window.vibe.orchestrator.enqueue({text:'Read ${session.handle}',origin:'text'})`); assert.equal(busy.ok, true); assert.equal(busy.status, 'queued'); assert(busy.requestId);
  ...
  const wrongHistory = toolsFrom(await command(`Resume Codex conversation ${first.id} in ${project.path}`), false)[0]; assert.equal(wrongHistory.ok, false, JSON.stringify(wrongHistory)); assert(!fs.existsSync(resumeLog));
  ...
  const followup=toolsFrom(await command('Tell Fixture history 1: FOLLOWUP_MUST_TARGET_RESUMED_CHAT'),false)[0];
  assert.equal(followup.ok,false,JSON.stringify(followup));
