...
(async()=>{
 try{
  let hooks=await list('default');assert.equal(app(hooks).length,7);assert.ok(app(hooks).every(h=>h.trustStatus==='trusted'&&h.enabled));
  console.log('native: seven exact generated hooks trusted');
  hooks=await list('custom',{config:custom,project:custom});assert.equal(hooks.length,9);assert.ok(hooks.filter(h=>!app(hooks).includes(h)).every(h=>h.trustStatus!=='trusted'));
