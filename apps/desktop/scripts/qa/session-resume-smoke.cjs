...
if (!process.versions.electron) {
  ...
  if (phase === 'reopen') for (const session of sessions.filter(s => ['claude', 'codex', 'open-codex'].includes(s.id))) {
    session.threadRef = { ...session.threadRef, id: `selected-C-${session.id}` };
  }
  const calls = [];
  ...
  async function inspect(win) {
    ...
    try {
      ...
      await wait(500);
      let persisted = await win.webContents.executeJavaScript(`JSON.parse(localStorage.getItem(${JSON.stringify(storageKey)}))`);
      verify(sessions, persisted, calls);
      if (phase === 'seed') {
        for (const [revision, label] of [[1, 'B'], [2, 'C']]) {
          for (const session of sessions.filter(s => ['claude', 'codex', 'open-codex'].includes(s.id))) {
            const ref = { ...session.threadRef, id: `selected-${label}-${session.id}` };
            const pending = session.id === 'open-codex';
            win.webContents.send('terminal:runtime', { id: session.id, generation: `fixture-${session.id}`, launchToken: session.launchToken,
              revision, provider: session.kind, cwd: session.cwd, processState: 'running', agentProcessState: 'running',
              turnState: pending ? 'unknown' : 'idle', observation: pending ? 'unavailable' : 'observed', telemetryHealth: 'available',
              children: [], activeTools: [], childActivity: false, updatedAt: Date.now(), binding: { status: pending ? 'pending' : 'found' },
              selection: { status: pending ? 'pending' : 'confirmed', revision, threadRef: ref }, ...(pending ? {} : { conversation: ref }) });
          }
          await until(() => win.webContents.executeJavaScript(`JSON.parse(localStorage.getItem(${JSON.stringify(storageKey)})).flatMap(w=>w.sessions).filter(s=>['claude','codex','open-codex'].includes(s.id)).every(s=>s.threadRef.id === 'selected-${label}-'+s.id)`), `hidden panes persist selection ${label}`);
        }
        persisted = await win.webContents.executeJavaScript(`JSON.parse(localStorage.getItem(${JSON.stringify(storageKey)}))`);
        assert.equal(persisted.flatMap(w => w.sessions).find(s => s.id === 'open-codex').threadSelectionPending, true);
      }
  ...
  ipcMain.handle = (channel, listener) => handle(channel, async (event, payload = {}) => {
    if (['terminal:create', 'fusion-chat:start', 'openfusion-chat:start', 'agent-thread:latest'].includes(channel)) {
      ...
      if (channel === 'agent-thread:latest') return payload.confirmId ? { status: 'found', rootVerified: true, threadRef: { provider: payload.provider, id: payload.confirmId, createdAt: 1700000000000, updatedAt: 1700000000001 } } : { status: 'missing' };
  ...
  if (process.env.LINA_RESUME_TEST_PACKAGE) app.setAppPath(process.env.LINA_RESUME_TEST_PACKAGE);
  require(path.join(process.env.LINA_RESUME_TEST_PACKAGE || root, 'backend/main.cjs'));
}
