...
// The drag preview is read once it stops changing, not after a fixed wait: a
// flat 120 ms sampled the pane's pre-drag rectangle on one slow frame and failed
// closeEnough against a drop that was correct. Settled means two consecutive
// samples agree and the rectangle has left where it started. On the cap the last
// sample is kept, so a preview that never moves still fails closeEnough.
const sameRect=(a,b)=>Boolean(a&&b)&&["left","top","width","height"].every(k=>Math.abs(a[k]-b[k])<1);
async function settledGeometry(id,before,cap=1500){const end=Date.now()+cap;let previous=null,current=await geometry(id);
  while(Date.now()<end){if(previous&&sameRect(previous,current)&&!sameRect(current,before))return current;await sleep(50);previous=current;current=await geometry(id);}
  return current;}
async function drag(id,dx,dy,shift=false,edge){const p=await point(selector(id)+(edge?` .pane-resize-edge-${edge}`:" .pane-title"));const modifiers=shift?8:0;const before=await geometry(id);await cdp.send("Input.dispatchMouseEvent",{type:"mousePressed",...p,button:"left",clickCount:1,modifiers});for(let i=1;i<=8;i++){await cdp.send("Input.dispatchMouseEvent",{type:"mouseMoved",x:p.x+dx*i/8,y:p.y+dy*i/8,buttons:1,modifiers});await sleep(35);}const preview=await settledGeometry(id,before);await cdp.send("Input.dispatchMouseEvent",{type:"mouseReleased",x:p.x+dx,y:p.y+dy,button:"left",clickCount:1,modifiers});await sleep(350);return {preview,committed:await geometry(id)};}
let stage=0;
async function seed(rects,live=false){stage++; const w=await cdp.eval(`document.querySelector('.tiled-board')?.clientWidth-20 || 1000`);const sessions=rects.map(r=>({id:r.id,name:r.id,kind:r.kind||"terminal",command:r.command||"",cwd:output,createdAt:Date.now(),started:live,nextLaunchMode:"new",launchToken:stage,status:"idle",layout:{x:r.left/w*100,y:r.top,w:r.width/w*100,h:r.height,unit:"fluid"}}));await cdp.eval(`(async()=>{for(const s of await window.vibe.terminal.getRuntimeSnapshots())await window.vibe.terminal.kill(s.sessionId||s.id);localStorage.setItem('vibe-terminal:workspaces:v2',${JSON.stringify(JSON.stringify([{id:"qa",name:"QA Board",path:output,sessions},{id:"hidden",name:"QA Hidden",path:root,sessions:[]}]))});localStorage.setItem('vibe-terminal:active-workspace:v1','qa');localStorage.setItem('vibe-terminal:active-view:v1','project');await ${require('./workspace-fixture.cjs').checkpointFromStorage};location.reload();})()`);await until(async()=>cdp.eval(`document.querySelectorAll('.pane-frame').length===${rects.length}`),"seed panes");await until(async()=>{if(stage===1)return cdp.eval(`(()=>{const b=document.querySelector(".tiled-board");return b.clientWidth>500;})()`);const currentWidth=await cdp.eval(`document.querySelector(".tiled-board").clientWidth-20`);for(const r of rects){const g=await geometry(r.id);if(!g||Math.abs(g.width-Math.max(280,r.width/w*currentWidth))>2||Math.abs(g.height-r.height)>2)return false;}return true;},"seed geometry settled");await sleep(stage===1?800:200);return await cdp.eval(`document.querySelector('.tiled-board').clientWidth-20`);}
