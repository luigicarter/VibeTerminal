...
test('a narrowed wrapped screen can grow and consume a full ConPTY redraw', { timeout: 2000 }, async () => {
  const observation = createTerminalObservation();
  const scope = { id: 'resized', generation: 'g' };
  try {
    await observation.ingest({ ...scope, type: 'created', cols: 100, rows: 28 });
    await observation.ingest({ ...scope, type: 'data', sequence: 1,
      data: '\x1b[2;36H' + 'x'.repeat(230) + '\r\n\r\nresult\r\n' + 'y'.repeat(110) });
    await observation.ingest({ ...scope, type: 'resize', cols: 52, rows: 10 });
    await observation.ingest({ ...scope, type: 'resize', cols: 109, rows: 28 });
    const lines = Array.from({ length: 28 }, (_, index) => `redraw-${index}`);
    await observation.ingest({ ...scope, type: 'data', sequence: 2, data: '\x1b[2J\x1b[H' + lines.join('\r\n') });
    const view = await observation.read(scope);
    assert.equal(view.text, lines.join('\n'));
    assert.deepEqual(view.cursor, { x: 9, y: 27 });
