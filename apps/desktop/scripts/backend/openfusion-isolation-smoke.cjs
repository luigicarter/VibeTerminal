...
async function main() {
  ...
  const nativeListings = mainSource.slice(mainSource.indexOf('ipcMain.handle("agent-thread:list"'),
    mainSource.indexOf('if (payload?.provider !== "opencode"'));
  for (const provider of ['claude', 'codex', 'open-codex']) assert(
    nativeListings.includes(`payload?.provider === "${provider}"`),
    `${provider} listing must pass through above the opencode fail-closed gate`
  );
