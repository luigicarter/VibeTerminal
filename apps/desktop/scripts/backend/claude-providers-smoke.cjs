...
process.env.LINA_MODEL_PROVIDERS_FILE = storeFile;
...
// ---- wiring contracts: preload surface, main IPC, menu, spawn injection ----
{
  ...
  assert.ok(
    main.includes("await resolveProviderEnv(payload.providerProfileId, payload.providerModelOverride)"),
  ...
  assert.ok(
    main.includes("claudeProviderGateways.environment(profileId, modelOverride)"),
