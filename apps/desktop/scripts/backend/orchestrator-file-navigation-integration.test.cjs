...
async function fixture(t){const temporaryParent=await fs.promises.realpath(os.tmpdir()),root=fs.mkdtempSync(path.join(temporaryParent,'vibe-file-navigation-')),steps=[],file=path.join(root,'source.txt');fs.writeFileSync(file,'x'.repeat(20000));let modelError;
  const relay=createOrchestrator({userDataPath:root,getRoots:()=>({projects:[root]}),interpretIntent:()=>({goal:'Read the requested source.',access:'read-only',actions:[]}),
    fetch:async(url,options)=>{try{if(url.endsWith('/key'))return Response.json({data:{}});if(url.endsWith('/models'))return Response.json({data:[{id:'fixture',context_length:128000,supported_parameters:['tools']}]});const next=steps.shift();assert(next,'Every model round is scripted');return Response.json(typeof next==='function'?next(JSON.parse(options.body)):next);}catch(error){modelError=error;throw error;}}});
  t.after(async()=>{await relay.dispose();assert.equal(path.dirname(root),temporaryParent);fs.rmSync(root,{recursive:true,force:true});});
  await relay.configure({apiKey:'fixture',model:'fixture',sessionOnly:true});await relay.setEnabled(true);return {relay,steps,file,error:()=>modelError};}
test('file cursors and committed bookmarks continue beyond directory paging limits',async t=>{
  ...
  for(let page=0;page<4;page++){
    ...
    const result=await f.relay.send({text:`Continue reading ${f.file}.`,origin:'text'});assert.equal(result.ok,true,f.error()?.stack||JSON.stringify(result));
  }
