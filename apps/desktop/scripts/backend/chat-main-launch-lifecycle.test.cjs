...
test('a blocked pane preparation cannot hold up an independent pane', async () => {
  ...
  // Fusion resolves provider settings before preparation; arrival order can
  // differ from request order. Release the intended pane, not an array slot.
  const secondPreparation = f.preparations.find(item => item.id === 'second');
  const firstPreparation = f.preparations.find(item => item.id === 'first');
  assert.ok(secondPreparation); assert.ok(firstPreparation);
  secondPreparation.gate.resolve(); assert.equal((await second).ok, true);
  assert.equal(f.effects.find(event => event.type === 'start').payload.id, 'second');
  firstPreparation.gate.resolve(); assert.equal((await first).ok, true);
});
