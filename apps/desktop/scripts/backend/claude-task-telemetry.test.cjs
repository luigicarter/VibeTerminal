...
async function fixture(t, engine = 'node', provider = 'claude', now = Date.now) {
  ...
  const hook = (name, fields = {}) => new Promise((resolve, reject) => {
    const type = { UserPromptSubmit: 'agent.running', PreToolUse: 'agent.running', PostToolUse: 'agent.running',
      PermissionRequest: 'agent.waiting', PostToolUseFailure: 'agent.running', SubagentStart: 'agent.subagent.started', SubagentStop: 'agent.subagent.stopped', Stop: 'agent.completed' }[name];
    const args = engine === 'powershell' ? ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', path.join(manager.runDir, 'notify.ps1')]
      : [observer];
    args.push(type);
    if (name === 'PermissionRequest') args.push('approval');
...
for (const engine of ['node', ...(process.platform === 'win32' ? ['powershell'] : [])]) {
  test(`Claude ${engine}: native-shaped missing-ID child approval survives unrelated authenticated tool callbacks`, async t => {
    const f = await fixture(t, engine);
    await f.hook('UserPromptSubmit');
    await f.hook('PermissionRequest', { agent_id: 'approval-child', tool_name: 'Bash', tool_input: { command: 'PRIVATE_APPROVAL_TEXT' } });
    const pending = () => f.state().children.find(c => c.id === 'approval-child');
    assert.equal(pending()?.attention?.reason, 'approval');
    await f.hook('PostToolUse', { agent_id: 'approval-child', tool_name: 'Read', tool_use_id: 'unrelated-return', tool_response: 'PRIVATE_RESULT' });
    assert.equal(pending()?.attention?.reason, 'approval');
    assert.equal(pending()?.approvals[0].identity, 'unidentified');
    assert.equal(JSON.stringify(f.events).includes('PRIVATE_APPROVAL_TEXT'), false);
  });
  test(`Claude ${engine}: unique native child attempt provides approval identity and its matching return settles it`, async t => {
    const f = await fixture(t, engine);
    await f.hook('UserPromptSubmit');
    await f.hook('PreToolUse', { agent_id: 'approval-child', tool_name: 'Bash', tool_use_id: 'native-attempt' });
    await f.hook('PermissionRequest', { agent_id: 'approval-child', tool_name: 'Bash' });
    assert.equal(f.state().children.find(c => c.id === 'approval-child')?.attention?.toolId, 'native-attempt');
    await f.hook('PostToolUse', { agent_id: 'approval-child', tool_name: 'Bash', tool_use_id: 'native-attempt' });
    assert.equal(f.state().children.find(c => c.id === 'approval-child')?.attention, undefined);
