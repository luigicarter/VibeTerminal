...
export const HISTORY_CONFIG_FIELDS = [
  "providerProfileId", "providerModelOverride", "openCodexModel", "fusionPlannerFamily", "fusionPlannerModel",
...
export function conversationLaunch(input: SavedConversation): { kind: AgentKind; patch: Partial<AgentSession> } {
  ...
  if (!conversation.id || !conversation.cwd || !["claude", "codex", "open-codex", "codex-web", "opencode", "cursor", "gemini", "kimi", "kimi-custom", "qwen", "grok"].includes(conversation.provider)) throw new Error("Saved conversation identity is incomplete.");
