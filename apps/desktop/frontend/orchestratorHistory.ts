...
export function conversationLaunch(input: SavedConversation): { kind: AgentKind; patch: Partial<AgentSession> } {
  ...
  if (!conversation.id || !conversation.cwd || !["claude", "codex", "open-codex", "opencode", "cursor", "gemini", "kimi", "kimi-custom", "qwen", "grok"].includes(conversation.provider)) throw new Error("Saved conversation identity is incomplete.");
