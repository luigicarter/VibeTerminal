...
export type VoicePlaybackAcknowledgments = { playbackStarted?: string; playbackDone?: string; playbackError?: string; playbackReplyId?: string };
export type VoiceApi = {
  getState(): Promise<VoiceState>; onState(cb: (state: VoiceState) => void): () => void;
  configure(patch: Record<string, unknown> & VoicePlaybackAcknowledgments): Promise<VoiceResult>;
