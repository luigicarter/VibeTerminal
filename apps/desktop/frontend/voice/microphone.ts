...
export class VoiceMicrophone {
  ...
  async start(onFrame: (samples: number[], sampleStart: number) => void, microphoneId?: string, onError?: (error?: Error) => void, onStall?: (message: string) => void) {
    ...
    // Leave input gain under the user's control instead of automatically adjusting it.
    const stream = await navigator.mediaDevices.getUserMedia({ audio: { ...(microphoneId ? { deviceId: { exact: microphoneId } } : {}), channelCount: 1, echoCancellation: true, noiseSuppression: true, autoGainControl: false }, video: false });
