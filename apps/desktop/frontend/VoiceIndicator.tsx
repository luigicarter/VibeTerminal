...
export default function VoiceIndicator() {
  ...
  const visual = needsAttention ? 'error' : !state.listening && !busy ? 'muted' : working ? 'thinking' : state.phase === 'speaking' ? 'speaking' : state.phase === 'recording' ? 'recording' : 'listening';
  ...
  return <div className={`voice-indicator voice-${visual}${hidden ? ' voice-hidden' : ''}`} onContextMenu={event => { event.preventDefault(); void act(() => api.configure({ menu: true })); }}>
    <button className="voice-mic" aria-label={`${status}. ${action}`} onPointerDown={event => { if (event.button !== 0) return; event.preventDefault(); void press(); }} onPointerUp={event => { if (event.button === 0) releasePointer(true); }} onPointerLeave={() => releasePointer()} onPointerCancel={() => releasePointer()} onKeyDown={event => { if (event.key === ' ') event.preventDefault(); if (event.key === 'Enter' && !event.repeat) { if (automatic) sendAutomatic(automaticIdentity()); else if (working) void act(() => api.cancelSpeech()); } }}>
      <span className="voice-hud" aria-hidden="true">
        <i className="voice-hud-segment" />
        <i className="voice-hud-segment" />
        <i className="voice-hud-segment" />
        <i className="voice-hud-segment" />
        <i className="voice-hud-scan" />
        <i className="voice-hud-pulse" />
        <i className="voice-hud-tick voice-hud-tick-n" />
        <i className="voice-hud-tick voice-hud-tick-e" />
        <i className="voice-hud-tick voice-hud-tick-s" />
        <i className="voice-hud-tick voice-hud-tick-w" />
      </span>
      {automatic ? <Send size={24} strokeWidth={1.7} aria-hidden="true"/> : working ? <Square size={19} strokeWidth={1.7} fill="currentColor" aria-hidden="true"/> : busy || state.listening ? <Mic size={25} strokeWidth={1.7} aria-hidden="true"/> : <MicOff size={25} strokeWidth={1.7} aria-hidden="true"/>}
    </button>
