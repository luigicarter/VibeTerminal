...
export const OPUS_LABEL = "Opus 5";
export const SONNET_LABEL = "Sonnet 5";
...
export function codexEffortValuesForModel(
  ...
  const live = liveCatalog?.codex?.find((option) =>
    !modelId || modelId === "auto" || modelId === "default" ? option.isDefault === true : option.id.trim().toLowerCase() === modelId
  );
...
> = {
  claude: [
    { id: "opus", label: "Opus · latest", desc: "Claude CLI alias · complex coding and review" },
    { id: "sonnet", label: "Sonnet · latest", desc: "Claude CLI alias · balanced speed and intelligence" },
    { id: "fable", label: "Fable · latest", desc: "Claude CLI alias · demanding reasoning" },
    { id: "claude-opus-5", label: OPUS_LABEL, desc: "Pinned version · complex coding and review" },
    { id: "claude-sonnet-5", label: SONNET_LABEL, desc: "Pinned version · balanced speed and intelligence" },
    { id: "claude-fable-5-1", label: "Fable 5.1", desc: "Pinned version · long-horizon reasoning" }
  ],
  codex: [
    { id: "auto", label: "Codex default", desc: "Use codex's configured default model" },
    { id: "gpt-6-astra", label: "GPT-6 Astra", desc: "Complex reasoning, coding, and long-running work" },
    { id: "gpt-5.6-sol", label: "GPT-5.6 Sol", desc: "General coding and professional work" },
    ...
    { id: "gpt-5.4-mini", label: "GPT-5.4 Mini", desc: "Lightweight model for focused work" }
  ]
};
...
export const FUSION_SLASH_COMMANDS: SlashCommand[] = [
  ...
  { name: "/mode", desc: "Toggle Auto/Plan mode", section: "Workflow" },
  {
    name: "/planner",
    desc: "Planner model, reasoning effort, and fast serving",
    section: "Models & reasoning",
    submenu: true
  },
  {
    name: "/executor",
    desc: "Executor model, reasoning effort, and fast serving",
    section: "Models & reasoning",
    submenu: true
  },
  { name: "/planner-model", desc: "Choose the planner family and model", picker: "planner", section: "Models & reasoning" },
  { name: "/executor-model", desc: "Choose the executor family and model", picker: "executor", section: "Models & reasoning" },
  { name: "/effort", desc: "Set reasoning depth for one or both roles", submenu: true, section: "Models & reasoning" },
  { name: "/speed", desc: "Quick, balanced, deep, or maximum reasoning", submenu: true, section: "Models & reasoning" },
  { name: "/fast", desc: "Faster serving at higher cost; keeps your models", submenu: true, section: "Models & reasoning" },
  { name: "/models", desc: "Show the current models and effort", section: "Models & reasoning" },
  { name: "/refresh-models", desc: "Reload available models from the runtimes", section: "Models & reasoning" },
  ...
  { name: "/clear", desc: "Start a fresh conversation", aliases: ["/new"], section: "Conversation" },
...
// Exact command/alias matches lead; otherwise search every word in the
// description too. Arguments never fall back to this top-level search.
export function searchSlashCommands<T extends { name: string; desc: string; aliases?: string[] }>(commands: T[], query: string): T[] {
  const token = query.trim().replace(/^\//, "").toLowerCase();
  const words = token.split(/\s+/).filter(Boolean);
  const score = (cmd: T) => {
    const names = [cmd.name, ...(cmd.aliases ?? [])].map((name) => name.replace(/^\//, "").toLowerCase());
    if (names.includes(token)) return 0;
    if (names.some((name) => name.startsWith(token))) return 1;
    return 2;
  };
  return commands.filter((cmd) => words.every((word) =>
    `${cmd.name} ${cmd.desc} ${(cmd.aliases ?? []).join(" ")}`.toLowerCase().includes(word)
  )).sort((a, b) => score(a) - score(b));
}

export const isSlashItemSelectable = (item: SlashMenuItem | undefined) =>
  Boolean(item && !item.disabled && (item.command || item.fill !== undefined));

export function moveSlashIndex(items: SlashMenuItem[], index: number, key: string): number {
  const selectable = items.map((item, i) => isSlashItemSelectable(item) ? i : -1).filter((i) => i >= 0);
  if (!selectable.length) return -1;
  if (key === "Home") return selectable[0];
  if (key === "End") return selectable[selectable.length - 1];
  const current = selectable.indexOf(index);
  const direction = key === "ArrowUp" || key === "PageUp" ? -1 : 1;
  if (current < 0) return direction > 0 ? selectable[0] : selectable[selectable.length - 1];
  const step = key === "PageDown" || key === "PageUp" ? 8 : 1;
  const next = step > 1 ? Math.max(0, Math.min(selectable.length - 1, current + direction * step))
...
export const effortItems = (
  ...
  const planner = context?.plannerEffort || "auto";
  const executor = context?.executorEffort || "auto";
  const current = scope === "planning" ? planner : scope === "execution" ? executor : planner === executor ? planner : null;
  return scopeEffortValues(scope, context).map((effort) => ({
...
const quickPlannerModelLabel = (context?: SlashMenuContext) =>
  contextPlannerFamily(context) === "codex" ? "GPT-5.6 Luna" : "Sonnet · latest";
...
export const filterSlashItems = (items: SlashMenuItem[], query: string) => {
  ...
  return items.filter((item) => normalized.split(/\s+/).every((word) =>
    `${item.label} ${item.desc} ${item.command ?? ""}`.toLowerCase().includes(word)
  ));
};
...
export function buildFusionPicker(
  ...
  if (!picker.family) {
    ...
    if (filter) {
      for (const family of FUSION_FAMILY_VALUES) {
        items.push(...filterSlashItems(familyModelRows(picker.role, family, context,
          (fam, modelId) => `__model:${fam}/${modelId}`), filter));
      }
    }
    return {
...
export function buildSlashMenu(input: string, context?: SlashMenuContext): SlashMenu {
  ...
  if (lower === "/fast" || lower.startsWith("/fast ")) {
    const { title, items } = submenu("/fast", "Fast serving", (["planner", "executor"] as const).flatMap((role) => {
      const enabled = role === "planner" ? context?.plannerFast : context?.executorFast;
      return [Boolean(enabled), !enabled].map((on) => ({
        key: `fast-${role}-${on}`,
        label: `${pickerRoleLabel(role)} — ${on ? "On" : "Off"}`,
        desc: on ? "Faster tokens, same model and effort; higher cost" : "Standard serving",
        current: Boolean(enabled) === on,
        section: pickerRoleLabel(role),
        command: `/fast ${role} ${on ? "on" : "off"}`
      }));
    }));
    return { title, items };
  }
  ...
  const commands = searchSlashCommands(FUSION_SLASH_COMMANDS, token);
  return {
    title: "Commands",
