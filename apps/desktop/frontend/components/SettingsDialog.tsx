  ...
  return <div className="confirmation-backdrop">
    <section className="confirmation-dialog settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-dialog-title" onClick={event=>event.stopPropagation()}>
      ...
      <div className="settings-body">
        <nav className="settings-navigation" aria-label="Settings sections">{[{id:'orchestrator',label:'Orchestrator & voice'},{id:'providers',label:'Models & providers'},{id:'appearance',label:'Appearance'},{id:'phone',label:'Phone'}].map(item=><button key={item.id} aria-current={panel===item.id?'page':undefined} onClick={()=>setPanel(item.id)}>{item.label}</button>)}</nav>
