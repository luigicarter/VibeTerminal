...
import { ChevronDown, Plus, RefreshCw, Search, Terminal, X } from 'lucide-react';
...
import { chatBootstrap, flushChatWorkspace } from '../chatPersistence';
...
export function ChatsSection({ project, projects, sessions, multi, profiles, onNew, onFocus, onOpen }: {
  ...
  const [rows, setRows] = useState<ChatRow[]>([]), [query, setQuery] = useState(''), [all, setAll] = useState(() => localStorage.getItem('lina:chats:all') === 'true');
  const [archived, setArchived] = useState(false), [note, setNote] = useState(''), [refreshing, setRefreshing] = useState(false), [opening, setOpening] = useState(''), [newMenu, setNewMenu] = useState(false);
  ...
  useEffect(() => { localStorage.setItem('lina:chats:all', String(all)); }, [all]);
  useEffect(() => { setNewMenu(false); }, [project?.id, multi]);
  async function refresh() {
    if (refreshing || !api) return;
    setRefreshing(true); setNote('');
    try { await flushChatWorkspace(); const result = await api.refresh(!all && !multi && project ? { cwd: project.path } : {}); if (mounted.current) { setNote(result.warnings.join(' ')); await load(); } }
    catch (error) { if (mounted.current) setNote(String(error)); }
    finally { if (mounted.current) setRefreshing(false); }
  }
  useEffect(() => { const timer = setTimeout(() => { void refresh(); }, 500); return () => clearTimeout(timer); }, [project?.id, multi, all]);
  // Native scanning runs behind the cached list; live panes never wait on it.
  const visible = useMemo(() => rows.filter(row => {
    if (Boolean(row.archived) !== archived) return false;
    const pane = sessions.find(s => s.id === row.paneId);
    const owner = row.projectId || projects.find(p => p.sessions.some(s => s.id === row.paneId))?.id;
    const cwd = row.conversation?.cwd || row.cwd;
    const normalized = (value?: string) => (value || '').replace(/\\/g, '/').replace(/\/$/, '').toLowerCase();
    if (!all && (multi ? owner && projects.some(p => p.id === owner) : owner ? owner !== project?.id : !project || normalized(cwd) !== normalized(project.path))) return false;
    return `${row.title} ${row.conversation?.title || ''} ${row.conversation?.id || ''} ${cwd || ''} ${pane?.name || ''}`.toLowerCase().includes(query.toLowerCase());
  }), [rows, sessions, projects, project, multi, all, archived, query]);
  const [limit, setLimit] = useState(50);
  useEffect(() => setLimit(50), [query, project?.id, multi, all, archived]);
  ...
  return <section className="chats-section" aria-label="Chats">
    <div className="sidebar-section-title">Chats <span className="sidebar-section-count">{visible.length}</span>
      <button className="chats-icon" aria-label="Refresh saved chats" title="Find saved chats in known folders" disabled={refreshing} onClick={() => void refresh()}><RefreshCw size={13} className={refreshing ? 'chats-spinning' : ''}/></button>
      <button className="chats-icon" aria-label="New chat" aria-expanded={newMenu} onClick={() => setNewMenu(value => !value)}><Plus size={16}/></button>
    </div>
    <div className="chats-filters"><button onClick={() => setAll(value => !value)} title="Switch between this project and all chats">{all ? 'All chats' : multi ? 'Multi mode' : project?.name || 'No project'} <ChevronDown size={12}/></button><button className={archived ? 'active' : ''} onClick={() => setArchived(value => !value)} aria-pressed={archived}>Archived</button></div>
    ...
    <div className="chats-list">
      {!visible.length && <p className="chats-empty">{query ? 'No matching chats.' : archived ? 'No archived chats.' : 'Your agent chats appear here. Use Refresh to find earlier saved chats.'}</p>}
      {visible.slice(0, limit).map(row => {
        ...
        return <div className="chat-row" key={row.chatId} data-chat-id={row.chatId}>
          <button className="chat-open" onClick={() => void open(row)} title={row.conversation?.cwd || row.cwd} disabled={Boolean(opening)}><Terminal size={15}/><span><span className="chat-title">{row.titleOverride ? row.title : row.conversation?.title || row.title}</span><span className="chat-meta">{provider} · {status}{all ? ` · ${projects.find(p => p.id === row.projectId)?.name || row.conversation?.cwd?.split(/[\\/]/).pop() || 'Multi'}` : ''}</span></span></button>
