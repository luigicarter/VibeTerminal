...
import { projectOrchestratorMessages } from "../orchestratorMessages";
export function OrchestratorPanel({ state, sessions, selectedId, onFocus, onSettings, projectPath, folders, setups, handoff, embedded = false, selectedTab, onTabChange }: {
    selectedTab?: string;
    ...
    const displayMessages = projectOrchestratorMessages(state?.messages || []);
    ...
    function taskControls(task: RelayTask) {
        return <div className="relay-task-meta"><span className={`relay-task-status ${task.status}`}>{task.status.replaceAll("-", " ")}</span><span title={taskLabel(task)}>{taskLabel(task)}</span>
            {!["finished", "failed", "cancelled", "continued", "paused"].includes(task.status) && <button type="button" onClick={() => void taskAction(() => relayApi()!.cancel({ requestId: task.requestId }))}>Cancel</button>}
            {task.controlDisposition !== "transferred" && ["paused", "failed"].includes(task.status) && <button type="button" onClick={() => void taskAction(() => relayApi()!.retry({ requestId: task.requestId }))}>{task.status === "paused" ? "Resume" : "Retry"}</button>}
            ...
            {task.continuedByRequestId && <small className="relay-task-waiting">Continued in a later request.</small>}
    ...
    const [targetSelection, setTargetSelection] = useState<{ id: string; projectPath?: string }>({ id: "" });
    const target = targetSelection.projectPath === projectPath && sessions.some(session => session.id === targetSelection.id) ? targetSelection.id : "";
    const setTarget = (id: string) => setTargetSelection({ id, projectPath });
    useEffect(() => { setTargetSelection({ id: "", projectPath }); }, [projectPath]);
    ...
    async function send() {
        ...
        setError("");
        try {
            const submittedProject = target ? undefined : projectPath;
            const result = await api.enqueue({ text: submittedText, origin: "text", ...(target ? { targetId: target } : {}), ...(submittedProject ? { projectPath: submittedProject } : {}), ...(submittedReply ? { replyToRequestId: submittedReply.requestId, questionId: submittedReply.questionId } : {}) });
            if (result.ok) {
    ...
    <div className="dock-content" role="tabpanel" hidden={!expanded}>
      {tab === "Orchestrator" && <div className="relay-view"><div className="relay-thread" ref={threadRef} onScroll={event => { const node = event.currentTarget; followThread.current = node.scrollHeight - node.scrollTop - node.clientHeight < 32; }}>{state?.requests?.filter(request=>!["resolved","cancelled"].includes(request.state)).map(request=><div className="relay-request" key={`${request.id}-${request.revision}`}><span>{sessions.find(session=>session.id===request.sessionId)?.name || "Session"} needs your {request.kind === "permission" ? "permission" : "answer"}</span>{request.questions?.map((question,index)=><p key={index}>{question.question}{question.options?.length ? <small>{question.options.map(option=>option.label).join(" · ")}</small> : null}</p>)}{request.detail && <p>{request.detail}</p>}<button onClick={()=>onFocus(request.sessionId)}>Review in session</button></div>)}{displayMessages.length ? displayMessages.map(message => {
            const task = message.requestId ? taskByRequest.get(message.requestId) : undefined;
            const sharedLabels = (message.relatedRequestIds || []).map(id => taskByRequest.get(id)).filter((item): item is RelayTask => Boolean(item)).map(item => `#${item.sequence}`);
            return <div className={`relay-message ${message.role}`} key={message.id}><span>{message.role === "user" ? "You" : message.origin === "monitor" ? "Activity report" : "Orchestrator"}{message.relatedRequestIds && message.relatedRequestIds.length > 1 ? ` · ${sharedLabels.join(", ") || "Shared result"}` : task && message.role !== "user" ? ` · ${taskLabel(task)}` : ""}</span><p>{message.text}</p>{task && message.role === "user" && taskControls(task)}</div>;
        ...
        <form className="relay-composer" onSubmit={event => { event.preventDefault(); void send(); }}><select aria-label="Command target" value={target} onChange={event => setTarget(event.target.value)}><option value="">{projectPath ? "Auto-assign · this project" : "Auto-assign"}</option>{sessions.map(session => <option value={session.id} key={session.id}>{session.name}</option>)}</select><textarea ref={composerRef} aria-label="Orchestrator instruction" placeholder={state?.ready ? "Tell Orchestrator what to do…" : "Connect a model in Settings to get started"} value={text} onChange={event => { textRevision.current++; setText(event.target.value); }} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) {
                event.preventDefault();
                void send();
      ...
      {tab === "Activity" && <div className="dock-activity">{sessions.map(session => <button key={session.id} className={`activity-session ${selectedId === session.id ? "active" : ""}`} onClick={() => onFocus(session.id)}><i className={`session-dot status-${session.status}`}/>{session.handle ? <em className="session-handle" title="What Lina calls this terminal">{session.handle}</em> : null}<span><strong>{session.name}</strong><small>{session.projectName || session.cwd}</small></span><span>{session.statusLabel || session.status.replaceAll("-", " ")}</span>{session.lastTool && <small>{session.lastTool}</small>}</button>)}{state?.receipts?.slice(-10).reverse().map(receipt => <p className="activity-receipt" key={receipt.id}><span>{receipt.status}</span> {receipt.text}</p>)}{!sessions.length && <p className="dock-note">Launch a session to see its live activity here.</p>}</div>}
      {tab === "History" && <ConversationHistory folders={folders} />}
      {tab === "Changes" && <ChangesPanel cwd={projectPath} />}
