...
import { configureTerminalLinks } from "../terminalLinks";
import { attachTerminalImages, bindTerminalImageDrop } from "../terminalImages";
...
const TITLE_REFRESH_PROVIDERS = new Set<string>(["claude", "codex", "open-codex", "kimi", "kimi-custom", "qwen", "grok"]);
...
export default function TerminalPane({
  ...
  useEffect(() => {
    if (session.kind !== 'codex-web') return;
    let lastError: string | undefined;
    return window.vibe?.codexWeb.onEvent(state => {
      if (state.id !== session.id || state.launchToken !== session.launchToken || !state.backgroundError || !state.error || state.error.reference === lastError) return;
      lastError = state.error.reference;
      terminalRef.current?.writeln(`\r\nCodex Web: ${state.error.message} [${state.error.reference}]`);
    });
  }, [session.id, session.kind, session.launchToken]);
  ...
  function armCodexStartingSettle() {
    if (
      !["codex", "open-codex"].includes(sessionRef.current.kind) ||
  ...
  function markActiveFromOutput() {
    ...
    if (["codex", "open-codex"].includes(sessionRef.current.kind)) {
  ...
  async function pasteImages(paths?: string[]) {
    const current = sessionRef.current, api = window.vibe?.codexWeb;
    if (current.kind !== 'codex-web' || !api || terminalExitedRef.current || !createdRef.current) return false;
    const pane = { id: current.id, launchToken: current.launchToken };
    return attachTerminalImages({ pane, paths, action: payload => api.action(payload),
      isCurrent: () => !terminalExitedRef.current && createdRef.current && sessionRef.current.id === pane.id && sessionRef.current.launchToken === pane.launchToken,
      paste: text => pasteText(text), onError: message => terminalRef.current?.writeln(`\r\nCodex Web: ${message}`) });
  }
  async function pasteClipboardText() {
    const text = window.vibe?.clipboard?.readText() ?? "";
    if (sessionRef.current.kind === 'codex-web') {
      const paths = window.vibe?.clipboard?.readFilePaths?.() || [];
      if (paths.length && paths.some(file => /\.(png|jpe?g|gif|webp)$/i.test(file))) { await pasteImages(paths); return true; }
      if (await pasteImages()) return true;
    }
  ...
  function handleTerminalContextMenu(event: React.MouseEvent<HTMLDivElement>) {
    ...
    void window.vibe.terminal.showContextMenu({
      ...
      imagePaste: session.kind === 'codex-web', launchToken: session.launchToken
    });
  }
  ...
  useEffect(() => {
    ...
    const cursorStyleHandler = ["codex", "open-codex", "codex-web"].includes(session.kind)
      ? configureCodexCursor(terminal)
      : undefined;
    const fitAddon = new FitAddon();
    terminal.loadAddon(fitAddon);
    configureTerminalLinks(terminal, session.kind === 'codex-web' ? filename => {
      const current = sessionRef.current;
      void window.vibe?.codexWeb.action({ id: current.id, launchToken: current.launchToken, action: 'open-image', filename })
        .then(result => { if (!result.ok) terminal.writeln(`\r\nCodex Web: ${result.error?.message || 'Could not open the image.'}`); })
        .catch(() => terminal.writeln('\r\nCodex Web: Could not open the image.'));
    } : undefined);
    ...
    const removeImageDrop = bindTerminalImageDrop(terminalHost, {
      enabled: () => sessionRef.current.kind === 'codex-web', filePath: file => window.vibe?.files?.getPathForFile?.(file) || '',
      drop: paths => { onSelect(); void pasteImages(paths); }
    });
    ...
    const removeContextMenuPasteListener =
      window.vibe?.terminal.onContextMenuPaste?.((payload) => {
        if (payload.id === session.id) {
          if (payload.launchToken !== undefined && payload.launchToken !== sessionRef.current.launchToken) return;
          if (sessionRef.current.kind === 'codex-web') void pasteClipboardText();
          else pasteText(payload.text);
        }
      });
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      window.vibe?.terminal.create({
        ...
        openCodexModel: session.openCodexModel,
  ...
  async function runTitleRefresh() {
    ...
    if (
      ...
      !threadId ||
      (provider !== "claude" && provider !== "codex" && provider !== "open-codex") ||
    ...
    >
      <header className="pane-header pane-drag-zone task-pane-header" title="Drag header to move pane">
        ...
        <PaneActions maximized={isMaximized} onMaximize={onMaximize} onClose={onClose}
          ...
          actions={[
            ...(session.kind === 'codex-web' ? [
              {title:'ChatGPT Web sign in',icon:<LogIn size={14}/>,run:()=>{ void window.vibe?.codexWeb.action({action:'login',id:session.id,launchToken:session.launchToken}).then(result=>{
                if (result.ok) onRestart();
                else terminalRef.current?.writeln(`\r\nCodex Web: ${result.error?.message || 'Sign-in could not start.'}`);
              }).catch(()=>terminalRef.current?.writeln('\r\nCodex Web: Could not reach Lina. Restart this terminal.')); }},
              {title:'Refresh Web models',icon:<RefreshCcw size={14}/>,run:()=>{ void window.vibe?.codexWeb.action({action:'refresh',id:session.id,launchToken:session.launchToken}).then(result=>{
                if (result.ok) onRestart();
                else terminalRef.current?.writeln(`\r\nCodex Web: ${result.error?.message || 'Models could not refresh.'}`);
              }).catch(()=>terminalRef.current?.writeln('\r\nCodex Web: Could not reach Lina. Restart this terminal.')); }},
              {title:'ChatGPT Web sign out',icon:<LogOut size={14}/>,run:()=>{ void window.vibe?.codexWeb.action({action:'logout',id:session.id,launchToken:session.launchToken}).then(result=>{
                terminalRef.current?.writeln(`\r\nCodex Web: ${result.ok ? 'Signed out. Use the pane menu to sign in again.' : result.error?.message || 'Sign-out could not finish.'}`);
              }).catch(()=>terminalRef.current?.writeln('\r\nCodex Web: Could not reach Lina. Restart this terminal.')); }},
            ] : []),
            ...
            ...(session.threadRef?.id || session.resumeRef?.id ? [{title:`Resume last ${profile.label} chat`,icon:<RotateCcw size={14}/>,run:onResume}] : [])
          ]}/>
