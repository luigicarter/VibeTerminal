...
export interface OpenCodexProvider { id: string; name: string; baseUrl: string; apiMode: 'auto' | 'responses' | 'chat-completions' | 'anthropic'; models: OpenCodexModel[]; hasKey: boolean; encrypted: boolean; }
export interface OpenCodexProviderList { profiles: OpenCodexProvider[]; models: OpenCodexConfiguredModel[]; defaultModel: string | null; }
export interface OpenCodexProvidersApi {
  list(): Promise<OpenCodexProviderList>;
  upsert(input: { id?: string; name: string; baseUrl: string; apiKey?: string; apiMode: string; models: OpenCodexModel[] }): Promise<{ ok: boolean; message?: string }>;
  remove(id: string): Promise<{ ok: boolean; message?: string }>;
  setDefault(key: string): Promise<{ ok: boolean; message?: string }>;
  discoverModels(input: { id?: string; baseUrl?: string; apiKey?: string; apiMode?: string }): Promise<{ ok: boolean; error?: string; models?: { id: string; label: string }[] }>;
}
export type ProviderModel = OpenCodexModel;
export type ConfiguredProviderModel = OpenCodexConfiguredModel;
export type ModelProvider = OpenCodexProvider;
export type ModelProviderList = OpenCodexProviderList;
export type ModelProvidersApi = OpenCodexProvidersApi;
...
export interface AgentSession {
  ...
  createdAt: number;
  threadRef?: AgentThreadRef;
  // A native clear/new/resume selected this ID; await exact root confirmation.
  threadSelectionPending?: boolean;
...
export interface CodeChangeSummary {
  ...
  branch?: string;
  upstream?: string;
  upstreamState?: "none" | "gone" | "tracked";
  ahead: number;
  behind: number;
  changedFiles: number;
  staged: number;
  ...
  updatedAt: number;
  message?: string;
  lineTotalsAvailable?: boolean;
  lineTotalsTruncated?: boolean;
}
...
// Working-tree changes and upstream divergence are independent observations.
