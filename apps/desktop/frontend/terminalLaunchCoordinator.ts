      ...
      for (const session of eligible.values()) {
        ...
        void Promise.resolve().then(async () => {
          ...
          if (session.threadSelectionPending && (!session.threadRef?.id || session.nextLaunchMode !== 'resume' || !options.confirmThread)) {
            options.onError(session, 'The latest chat selection is unavailable. Use Restart to start a fresh chat.');
            return;
          }
          if (options.confirmThread && isThreadedAgentKind(session.kind) &&
            ...
            if (!current(entry)) return;
            if (session.threadSelectionPending && confirmation?.status !== 'missing' &&
                !(confirmation?.status === 'found' && confirmation.rootVerified === true && confirmation.threadRef.id === session.threadRef.id)) {
              options.onError(session, 'The newly selected chat could not be verified. Use Restart to retry.');
              return;
            }
