...
function windowsPowerShellShimSource(provider) {
  return [
    ...
    "$PreviousInvocationId = $env:VIBE_TERMINAL_INVOCATION_ID",
    "$env:VIBE_TERMINAL_INVOCATION_ID = $InvocationId",
    "$ExitCode = 0",
    ...
    "}",
    "",
    "$env:VIBE_TERMINAL_INVOCATION_ID = $PreviousInvocationId",
...
const event = { type, sessionId, launchNonce, timestamp: observedAt, ...metadata,
  invocationId: process.env.VIBE_TERMINAL_INVOCATION_ID };
...
process.stdin.on("end", () => {
  ...
  const body = JSON.stringify({
    ...
    phase: hook.hook_event_name === "SessionStart" || hook.hook_event_name === "PreToolUse" ? "start" : hook.hook_event_name === "PostToolUse" ? "stop" : undefined,
    source: hook.hook_event_name === "SessionStart" && ["startup", "resume", "clear", "compact"].includes(hook.source) ? hook.source : undefined,
    transcriptPath: typeof hook.transcript_path === "string" ? hook.transcript_path : undefined,
    parentThreadId: typeof hook.parent_session_id === "string" ? hook.parent_session_id : undefined,
    invocationId: process.env.VIBE_TERMINAL_INVOCATION_ID,
...
function windowsNotifyPs1Source() {
  ...
  return [
    ...
    "    launchNonce = $env:VIBE_TERMINAL_LAUNCH_NONCE",
    "    timestamp = $ObservedAt",
    "    invocationId = $env:VIBE_TERMINAL_INVOCATION_ID",
    "  }",
    "  if ($Detail) {",
...
function buildClaudeSettingsJson(scriptPath, isWin) {
  ...
  const settings = {
    hooks: {
      SessionStart: [{ matcher: "startup|resume|clear|compact", hooks: [hook("agent.session")] }],
      ...
      server = http.createServer((request, response) => {
        ...
        request.on("end", () => {
          try {
            ...
            const textMetadata = ["providerThreadId", "providerTurnId", "toolId", "toolName", "taskId", "taskLabel", "parentThreadId", "transcriptPath", "cwd", "notificationType", "kind", "lifecycle", "invocationId"];
