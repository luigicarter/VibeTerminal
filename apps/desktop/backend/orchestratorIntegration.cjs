...
function createSessionDirectory({ getRuntime, now = Date.now } = {}) {
  ...
  function updateUi(items, roots = [], catalog = [], projectCatalog = []) {
    ...
    for (const s of Array.isArray(items) ? items : []) if (s?.id) ui.set(s.id, {
      ...
      fusionPlannerFamily: s.fusionPlannerFamily, fusionExecutorFamily: s.fusionExecutorFamily,
      fusionPlannerModel: s.fusionPlannerModel, fusionExecutorModel: s.fusionExecutorModel,
      openFusionPlannerModel: s.openFusionPlannerModel, openFusionExecutorModel: s.openFusionExecutorModel,
      fusion: s.fusion, openFusion: s.openFusion
    });
  }
    ...
    if (message.type === "start") {
      ...
      if (!old || old.signature !== signature && !attachesCurrentConversation || old.closed) {
        ...
        chats.set(p.id, { id: p.id, kind, cwd: p.cwd, ...(p.launchToken !== undefined && { launchToken: p.launchToken }), generation: randomUUID(), revision: 1,
          ...
          executorProvider: p.executorFamily, executorModel: p.executorModel,
...
function installOrchestrator(options) {
  ...
  const history = createOrchestratorHistoryProcess({ getConfig: () => {
    ...
    for (const cwd of roots) for (const provider of ["codex", "open-codex", "claude", "claude-custom", "cursor", "gemini", "kimi", "kimi-custom", "qwen", "grok", "opencode", "openfusion", "fusion"]) scopes.push({ provider, cwd });
