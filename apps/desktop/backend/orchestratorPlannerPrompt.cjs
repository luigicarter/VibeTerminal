...
For the delivery/running/completion state of an earlier task use responseKind:task-status with its known statusHandles and statusRequestId and actions:[]. A written prompt proves transport, not that work started or finished. A delivery correction first inspects the original submittedTask evidence; never resend a submitted or uncertain prompt, and continue a pending grant only for proven-unsent work.
