...
function createTerminalRuntime({ emit = () => {}, now = Date.now, lookup, capabilities = () => ({}) } = {}) {
  ...
  function publish(record) {
    if (record.approvals) {
      record.approvals.retainChildren(new Set(record.snapshot.children.map(child => child.id)));
      for (const child of record.snapshot.children) {
        child.approvals = record.approvals.waiting(`child:${child.id}`);
        if (child.approvals.length) child.attention = child.approvals[0];
        else if (child.attention?.reason === 'approval') child.attention = undefined;
      }
      record.snapshot.approvals = record.approvals.waiting('root');
      if (record.snapshot.approvals.length && !record.rootIdentityConflict && record.snapshot.processState === 'running' &&
          !['exited', 'failed'].includes(record.snapshot.agentProcessState)) {
        record.snapshot.attention = record.snapshot.approvals[0];
        record.snapshot.turnState = 'waiting';
      } else if (record.snapshot.attention?.reason === 'approval') record.snapshot.attention = undefined;
    }
  ...
  function beginLaunch(payload) {
    ...
    const record = {
      ...
      approvals: provider === 'claude' ? createApprovalLedger() : null,
      identityHints: new Map(), pendingEvents: [], selectionEvents: [], retiredTurnIds: new Set(), retiredConversations: new Set(), resolvedQuestionToolIds: new Set(), nextLookupAt: 0, lookupFailures: 0,
      nativeActive: false, pendingPriorTurnId: undefined, settledToolIds: new Set(), childStops: new Map(),
      claudeNativeChildren: new Set(), claudeBackgroundChildren: new Set(), claudeTaskSnapshotAt: 0, retiredTaskSnapshots: new Map(),
      ...
      snapshot: {
        ...
        ...(payload.providerProfileId && { providerProfileId: payload.providerProfileId }),
  ...
  function selectConversation(record, event) {
    ...
    record.selectionObservedAt = event.observedAt;
    const previousId = s.selection?.threadRef?.id || s.conversation?.id;
    if (previousId === event.providerThreadId && !record.rootIdentityConflict) return;
    if (previousId) {
      record.retiredConversations.add(previousId);
      while (record.retiredConversations.size > 256) record.retiredConversations.delete(record.retiredConversations.values().next().value);
    }
    record.retiredConversations.delete(event.providerThreadId);
    for (const child of s.children) child.ownerConversationId ||= child.providerThreadId === event.providerThreadId ? event.providerThreadId : s.conversation?.id || previousId;
    if (s.turnId) record.retiredTurnIds.add(s.turnId);
    clearInput(record);
    clearActiveTools(record);
    record.approvals?.clear('root');
    for (const key of ['turnId', 'turnStartedAt', 'turnEndedAt', 'attention', 'lastTool', 'approvals']) s[key] = undefined;
    s.turnState = 'unknown';
    s.activityObserved = false;
    s.observation = 'unavailable';
    record.nativeActive = false;
    record.rootIdentityConflict = false;
    record.transcriptPath = event.transcriptPath;
    record.identityHints.clear();
    // Callback HTTP arrival can lag a later UserPromptSubmit from the same
    // invocation. Retain source-time-ordered events for this exact candidate.
  ...
  function observeChild(record, event, eventAt) {
    ...
    if (entry?.ownerConversationId && ![event.providerThreadId, event.parentThreadId].includes(entry.ownerConversationId) &&
        event.providerThreadId !== entry.id && event.providerThreadId !== entry.providerThreadId) return undefined;
    ...
    if (!entry) {
      ...
      if (record.snapshot.selection && record.snapshot.conversation?.id) entry.ownerConversationId = record.snapshot.conversation.id;
    ...
    if (event.providerThreadId && event.providerThreadId !== record.snapshot.conversation?.id) {
      ...
      if (record.retiredConversations.has(event.providerThreadId)) entry.ownerConversationId = event.providerThreadId;
  ...
  function authoritativeChildEnd(record, event) {
    ...
    if (entry?.ownerConversationId && ![event.providerThreadId, event.parentThreadId].includes(entry.ownerConversationId) &&
        event.providerThreadId !== entry.id && event.providerThreadId !== entry.providerThreadId) return true;
  ...
  function claudeTaskRoot(record, event) {
    return record.snapshot.provider === "claude" && Boolean(event.providerThreadId) && Number.isFinite(event.observedAt) &&
      (event.providerThreadId === record.snapshot.conversation?.id ||
        record.snapshot.children.some(child => child.ownerConversationId === event.providerThreadId));
  }
  function observeClaudeTaskResult(record, event, eventAt) {
    ...
    const resultChild = record.snapshot.children.find(child => child.id === result.agentId);
    if (resultChild?.ownerConversationId && resultChild.ownerConversationId !== event.providerThreadId) return;
  ...
  function reconcileClaudeBackgroundTasks(record, event, eventAt) {
    const retiredRoot = record.retiredConversations.has(event.providerThreadId);
    const previousAt = retiredRoot ? record.retiredTaskSnapshots.get(event.providerThreadId) || 0 : record.claudeTaskSnapshotAt;
    if (!claudeTaskRoot(record, event) || (isChild(record, event) && !retiredRoot) || eventAt <= previousAt) return;
    ...
    for (const child of [...record.snapshot.children]) {
      if ((child.ownerConversationId || record.snapshot.conversation?.id) !== event.providerThreadId) continue;
    ...
    if (retiredRoot) {
      record.retiredTaskSnapshots.set(event.providerThreadId, eventAt);
      if (record.retiredTaskSnapshots.size > 256) record.retiredTaskSnapshots.delete(record.retiredTaskSnapshots.keys().next().value);
    } else record.claudeTaskSnapshotAt = eventAt;
  }
  ...
  function ingest(event) {
    ...
    const selectionEvent = selectedConversationEvent(record, event);
    if (selectionEvent) {
      selectConversation(record, event);
      if (selectionEvent === 'unordered') {
        record.retiredConversations.add(event.providerThreadId);
        s.selection = { status: 'unavailable', revision: s.selection.revision };
        s.binding = { status: 'ambiguous', message: 'The latest chat selection could not be ordered. Select a chat again.' };
      }
      return publish(record);
    }
    if (event.type.startsWith('agent-') && event.type !== 'agent-process' && event.providerThreadId &&
        event.invocationId === record.rootProcessId && record.rootProcessId &&
        !event.parentThreadId && event.transcriptKind !== 'subagent' && !event.taskId &&
        event.providerThreadId !== s.conversation?.id) {
      record.selectionEvents.push(event);
      if (record.selectionEvents.length > 128) record.selectionEvents.shift();
    }
    if (s.selection && event.type.startsWith('agent-') && event.type !== 'agent-process' &&
        event.providerThreadId === s.selection.threadRef?.id && !event.parentThreadId && event.transcriptKind !== 'subagent' &&
        !event.taskId && event.observedAt < record.selectionObservedAt) return null;
    // Old root callbacks remain historical. Identified background children may
    // still settle, but cannot make their former root current again.
    if (record.retiredConversations.has(event.providerThreadId) && !event.taskId &&
        !event.parentThreadId && event.transcriptKind !== 'subagent') {
      if (event.type === 'agent-activity') observeClaudeTaskResult(record, event, event.observedAt || now());
      if (event.type === 'agent-attention' && event.attention?.state === 'completed') reconcileClaudeBackgroundTasks(record, event, event.observedAt || now());
      return publish(record);
    }
    if (s.selection && s.selection.status !== 'confirmed' && event.type.startsWith('agent-') && event.type !== 'agent-process' &&
        !event.parentThreadId && event.transcriptKind !== 'subagent' && !event.taskId) {
      if (event.providerThreadId === s.selection.threadRef?.id) {
    ...
    switch (event.type) {
      ...
      case "agent-running":
        if (child) {
          const entry = observeChild(record, event, eventAt);
          if (entry) {
            record.approvals?.observe(`child:${entry.id}`, event, eventAt);
            if ((!record.approvals || entry.attention?.reason !== 'approval') &&
                (event.turnStart !== false || !entry.attention?.toolId || event.toolId === entry.attention.toolId)) entry.attention = undefined;
          }
          break;
        }
        ...
        if (newTurn) {
          if (event.turnStart !== false && !event.toolId && !event.toolName || event.providerTurnId && s.turnId && event.providerTurnId !== s.turnId) record.approvals?.clear('root');
      ...
      case "agent-attention": {
        if (child) {
          if (event.attention?.state === "waiting") {
            const entry = observeChild(record, event, eventAt);
            if (entry) record.approvals?.observe(`child:${entry.id}`, event, eventAt);
        ...
        if (event.providerTurnId && s.turnId && event.providerTurnId !== s.turnId) {
          const notifyOnlyCompletion = ["codex", "open-codex"].includes(s.provider) && s.capabilities?.finalCompletion === "authoritative" &&
        ...
        if (attention.state !== 'waiting' && !provisionalCompletion) record.approvals?.clear('root');
      ...
      case "agent-activity": {
        ...
        if (child) {
          ...
          if (entry && !childEndIsStale(record, event, eventAt)) record.approvals?.observe(`child:${entry.id}`, event, eventAt);
          if (entry?.attention && (!record.approvals || entry.attention.reason !== 'approval') && event.phase === "stop" && event.toolId === entry.attention.toolId) entry.attention = undefined;
          break;
        }
        ...
        if (event.phase === 'start') record.approvals?.observe('root', event, eventAt);
  ...
  async function refreshRecord(record) {
    const s = record.snapshot;
    if (s.selection?.status === 'unavailable') return;
    ...
    const selectionRevision = s.selection?.revision;
    try {
      if (s.selection?.status === 'pending') {
        const selection = s.selection;
        // Do not reuse a cached negative read: a new transcript may appear just
        // after the native hook. This path never scans for a different ID.
        const result = await lookup({ provider: s.provider, cwd: s.cwd, claudeHome: record.claudeHome,
          confirmId: selection.threadRef.id, transcriptPath: record.transcriptPath });
        if (!current(s.id, generation) || s.selection !== selection) return;
        if (owned(s.provider, selection.threadRef.id, s.id, record.claudeHome)) {
          s.binding = { status: 'ambiguous', message: 'The selected chat already belongs to another open pane.' };
        } else if (result?.status === 'found' && result.rootVerified === true &&
            result.threadRef?.id === selection.threadRef.id && !result.threadRef.parentThreadId && bind(record, result.threadRef)) {
          s.selection = { ...selection, status: 'confirmed', threadRef: s.conversation };
          s.children = s.children.filter(child => child.id !== selection.threadRef.id);
          record.selectionEvents = record.selectionEvents.filter(e => e.providerThreadId !== selection.threadRef.id);
          s.turnState = 'idle';
          s.observation = 'observed';
          record.lookupFailures = 0;
          replayPending(record);
        } else {
          s.binding = { status: 'pending', message: 'Waiting for the selected chat to be saved and verified.' };
        }
        publishMetadata();
        return;
      }
      ...
      if (!current(s.id, generation) || record.rootIdentityConflict || s.selection?.revision !== selectionRevision) return;
      ...
      record.lookupFailures += 1;
      record.nextLookupAt = now() + Math.min(60000, 8000 * 2 ** Math.min(record.lookupFailures, 3));
      if (current(s.id, generation) && s.conversation?.id === knownId && s.selection?.revision === selectionRevision) {
        backgroundUnavailable(record);
        s.binding = { status: knownId ? "found" : "unavailable", message: error.message };
        publishMetadata();
      }
    } finally {
      record.lookupInFlight = false;
      if (current(s.id, generation) && s.selection?.status === 'pending' && s.selection.revision !== selectionRevision) {
        record.nextLookupAt = 0;
        queueMicrotask(() => { void refreshRecord(record); });
      }
    }
  }
