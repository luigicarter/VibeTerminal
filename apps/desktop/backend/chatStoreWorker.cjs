...
process.on('message', message => {
  chain = chain.then(async () => {
    try {
      ...
      if (!['bootstrap', 'checkpoint', 'observe', 'observeChat', 'list', 'get', 'importConversations', 'update', 'confirmed', 'draft', 'backup', 'backupIfDue', 'finish', 'scopes', 'saveCopy', 'readCopy'].includes(message.method)) throw new Error('Unknown chat operation.');
