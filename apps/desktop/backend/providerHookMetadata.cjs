...
function hookMetadata(input) {
  ...
  if (name === 'SessionStart' && ['startup', 'resume', 'clear', 'compact', 'fork'].includes(hook.source)) {
    output.phase = 'start';
    output.source = hook.source;
  }
  ...
  if (output.phase && name !== 'SessionStart') output.kind = "tool";
...
function powershellHookMetadata() {
  return [
    ...
    "  if ($hookName -eq 'SessionStart' -and @('startup','resume','clear','compact','fork') -contains $hook.source) { $payload['phase'] = 'start'; $payload['source'] = [string]$hook.source }",
