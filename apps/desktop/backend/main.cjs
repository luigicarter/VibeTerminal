// A separate Electron process owns Codex Web's isolated browser and bridge.
if (process.argv.includes('--codex-web-host')) {
  require('./codexWebLauncher.cjs').bootstrap();
  return;
}
const {
  app,
...
function getCodexWebHost() {
  return codexWebHost ||= require('./codexWebHost.cjs').createCodexWebHost({
    ...
    broadcast: (channel, payload) => { if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send(channel, payload); }
  });
}
ipcMain.handle('codex-web:action', async (event, payload) => {
...
const userDataOverride = process.env.LINA_USER_DATA_DIR?.trim();
if (userDataOverride && !path.isAbsolute(userDataOverride)) throw new Error('LINA_USER_DATA_DIR must be an absolute directory.');
const existingUserDataPath = userDataOverride || app.getPath("userData");
...
function getOpenCodexRuntime() {
  return openCodexRuntime ||= createOpenCodexRuntime({ userData: app.getPath('userData'),
...
async function resolveProviderEnv(profileId, modelOverride) {
  const env = await claudeProviderGateways.environment(profileId, modelOverride);
  if (!env) {
    return null;
  }
...
function getAppIconPath() {
  return path.join(__dirname, "..", "frontend", "assets", "lina-logo.ico");
}
...
function ingestTelemetryEvent(event) {
  ...
  if (event?.generation !== undefined) {
    ...
    if (event.provider === 'open-codex' && event.type === 'agent-process' && event.phase === 'exit' &&
        event.processId && event.processId === record?.rootProcessId) void openCodexRuntime?.release(event.id, event.generation);
    if (record?.identityHints.size || record?.snapshot.selection?.status === 'pending') void terminalRuntime.refreshRecord(record);
  }
...
function releaseTerminalResources(sessionId, generation) {
  void openCodexRuntime?.release(sessionId, generation);
...
async function listFusionModelCatalog(family, refresh = false) {
  ...
  if (!refresh && cached && Date.now() - cached.fetchedAt < FUSION_MODEL_CATALOG_TTL_MS) {
...
function createMainWindow() {
  ...
  // Send links to the browser while keeping remote content out of windows
  // carrying the workspace preload. Terminal links also use this opener via IPC.
...
app.whenReady().then(() => {
  ...
  createMainWindow();
  // Warm an existing Codex Web account when Lina opens, without delaying paint
  // or waiting for the user to create a Codex Web pane. Fresh profiles are skipped.
  setImmediate(() => { void Promise.resolve().then(() => getCodexWebHost().prewarm()).catch(() => {}); });
...
ipcMain.handle("terminal:create", (_event, payload) => observedLaunches.run('terminal', payload, async () => {
  ...
  if (payload?.provider === 'codex-web') {
    ...
    instrumentation.env[pathKey] = web.binDir + path.delimiter + (instrumentation.env[pathKey] || process.env.PATH || '');
  }
  ...
  if (payload?.provider === 'open-codex') {
    const prepared = await getOpenCodexRuntime().prepare({ ...payload, generation: admission?.generation,
      resumeId: /^open-codex\s+resume\s/.test(payload.command || '') ? payload.threadRef?.id : undefined });
    if (admission && !runtime.isCurrent(payload.id, admission.generation)) {
      await getOpenCodexRuntime().release(payload.id, admission.generation);
      return { ok: false, cancelled: true, generation: admission.generation };
    }
    instrumentation = { ...instrumentation, stripEnv: prepared.stripEnv, env: { ...instrumentation?.env, ...prepared.env } };
    payload = { ...payload, command: prepared.command };
  }
  ...
  if (payload?.providerProfileId && payload.provider !== 'open-codex') {
    const providerEnv = await resolveProviderEnv(payload.providerProfileId, payload.providerModelOverride);
    if (!providerEnv) {
      if (payload?.id) {
        broadcastTerminalEvent({
          id: payload.id,
    ...
    // The shared gateway already validated and pinned the requested model.
  }
...
ipcMain.handle("fusion-chat:start", (_event, payload) => observedLaunches.run('fusion', payload, async () => {
  ...
  try {
    ...
    if (plannerFamily === "claude" || executorFamily === "claude") {
      providerEnv =
        await resolveProviderEnv(payload.providerProfileId || "default-custom") ||
        undefined;
...
ipcMain.handle("fusion-model-catalog:list", async (_event, payload) => {
  ...
  try {
    return await listFusionModelCatalog(family, payload?.refresh === true);
  } catch (error) {
    return {
      ok: false,
      family,
...
ipcMain.handle('model-providers:list', () => modelProviders.listProfiles());
ipcMain.handle('model-providers:upsert', (_event,payload) => { const result=modelProviders.upsertProfile(payload); if(result.ok)invalidateProviderModelCaches(); return result; });
ipcMain.handle('model-providers:delete', (_event,payload) => { const result=modelProviders.deleteProfile(payload?.id); if(result.ok)invalidateProviderModelCaches(); return result; });
ipcMain.handle('model-providers:set-default', (_event,payload) => { const result=modelProviders.setDefaultModel(payload?.key);if(result.ok)invalidateProviderModelCaches();return result; });
ipcMain.handle('model-providers:models', (_event,payload) => modelProviders.discoverModels(payload));
ipcMain.handle('open-codex-providers:list', () => openCodexProviders.listProfiles());
ipcMain.handle('open-codex-providers:upsert', (_event, payload) => openCodexProviders.upsertProfile(payload));
ipcMain.handle('open-codex-providers:delete', (_event, payload) => openCodexProviders.deleteProfile(payload?.id));
ipcMain.handle('open-codex-providers:set-default', (_event, payload) => openCodexProviders.setDefaultModel(payload?.key));
ipcMain.handle('open-codex-providers:models', (_event, payload) => openCodexProviders.discoverModels(payload));
...
ipcMain.handle("claude-providers:models", async () => {
  const { profiles } = modelProviders.listProfiles();
  const providers = profiles.map(profile => ({ providerId:profile.id,name:profile.name,models:profile.models.map(model=>({id:model.id,label:model.label})) }));
...
// Open a web link in the user's default browser. Restricted to
...
async function openExternalUrl(rawUrl) {
  ...
  try {
    ...
    return { ok: false, error: "could not open link in the default browser" };
  }
}

ipcMain.handle("app:open-external", (_event, payload) => openExternalUrl(payload?.url));
...
ipcMain.handle("terminal:show-context-menu", (event, payload = {}) => {
  ...
  const hasClipboardImage = payload.imagePaste === true && (!clipboard.readImage().isEmpty() || clipboard.availableFormats().includes('FileNameW'));
  ...
  const menu = Menu.buildFromTemplate([
    ...
    {
      label: "Paste",
      enabled: Boolean(sessionId && (hasClipboardText || hasClipboardImage)),
      click: () => {
        if (!sessionId) {
          return;
        }
