...
function createWorkspaceExecutor({ getSessions, getRequests, getEpoch, isDisposed, active, agentServices,
  ...
  async function doAction(raw, { intent, scope, diagnosticContext = {}, requestContext, token = getEpoch(), signal = requestContext?.controller?.signal } = {}) {
    ...
    check();
    if (Object.hasOwn(agentTools.OPERATIONS, action.kind)) {
      const descriptor = agentTools.OPERATIONS[action.kind];
      if (!agentServices || !(typeof agentServices.enabled === 'function' ? agentServices.enabled() : agentServices.enabled)) throw new Error('The agent record harness is not enabled for this request.');
      if (Object.keys(action).some(key => key !== 'kind' && !descriptor.fields.includes(key)) ||
          descriptor.required?.some(key => action[key] === undefined)) throw new Error('Invalid agent record arguments.');
      await requireFreshSessions(); check();
      if (agentServices.error()) throw new Error(agentServices.error());
      const { directory, queries, store } = agentServices;
      const record = action.agentId && directory.get(action.agentId);
      if (action.agentId && !record) throw new Error('Find the agent before reading or updating its record.');
      const owner = requestContext?.job;
      const reading = kind => value => {
        if (value?.ok && owner && record) { owner.agentReads ||= new Set(); owner.agentReads.add(record.agentId); }
        return intent && agentTools.CONTENT_READS.includes(kind) ? intent.readBudget.projectRead(redact(value), { tail: false }) : redact(value);
      };
      if (action.kind === 'find_agents') return redact(queries.find(action));
      if (action.kind === 'read_agent') return reading(action.kind)(queries.read(action));
      if (action.kind === 'read_work_item') return reading(action.kind)(queries.readWork(action));
      if (action.kind === 'record_agent_note') {
        if (!owner?.agentReads?.has(action.agentId) || !owner.task?.requestId || !diagnosticContext.toolCallId) throw new Error('Read this agent in the current request before recording a note.');
        const work = action.workItemId && workItems.get(action.workItemId);
        const knownWork = !action.workItemId || work && record.work.items.some(w => w.id === work.id);
        check();
        await store.syncIdentities(directory.exportIdentities()); check();
        return redact(await store.write({ id: action.noteId, agentId: action.agentId, workItemId: action.workItemId,
          kind: action.noteKind, body: action.text, expectedRevision: action.expectedRevision, operationId: diagnosticContext.toolCallId },
          { requestId: owner.task.requestId, knownAgent: true, knownWorkItem: Boolean(knownWork) }));
      }
      if (action.kind === 'read_agent_history') {
        const identity = record.identity, savedId = identity.native?.id;
        if (!savedId || !identity.native.provider || identity.state === 'ambiguous') return { ok: false, status: 'identity-unavailable', agentId: record.agentId };
        const nested = { intent, scope, diagnosticContext, requestContext, token, signal };
        let reference = action.reference;
        const matches = candidate => {
          const normalize = value => value ? JSON.parse(require('./orchestratorRouting.cjs').nativeKey({ provider: '_', home: '_', workspace: String(value), id: '_' }))[2] : '';
          const customProfile = identity.native.home?.startsWith('custom:') ? identity.native.home.slice(7) : undefined;
          return candidate?.id === savedId && normalize(candidate.cwd) === normalize(identity.cwd) &&
            (identity.kind !== 'fusion' || candidate.plannerProvider === identity.native.provider) &&
            (candidate.provider === identity.kind || identity.kind === 'claude' && candidate.provider === 'claude-custom') &&
            (customProfile ? candidate.providerProfileId === customProfile : candidate.claudeHome !== 'custom');
        };
        if (!reference) {
          const listed = await doAction({ kind: 'list_conversations', provider: identity.native.home?.startsWith('custom:') ? 'claude-custom' : identity.kind,
            cwd: identity.cwd, query: savedId, limit: 20 }, nested); check();
          const candidates = (listed.conversations || []).filter(matches);
          if (candidates.length !== 1) return { ok: false, status: 'history-unavailable', agentId: record.agentId, error: 'The exact agent conversation could not be uniquely found in its provider store.' };
          reference = candidates[0].reference;
        }
        if (!matches(historyCandidates.get(reference))) throw new Error('This history reference belongs to another agent or provider store.');
        const result = await doAction({ kind: action.query ? 'search_conversation' : 'read_conversation', reference,
          ...(action.query && { query: action.query }), ...(action.cursor && { cursor: action.cursor }), ...(action.maxChars && { maxChars: action.maxChars }) }, nested);
        check();
        const current = directory.get(record.agentId);
        if (!current || JSON.stringify(current.identity.native) !== JSON.stringify(identity.native) || current.identity.state === 'ambiguous') return { ok: false, status: 'conversation-changed' };
        if (result.ok && owner) { owner.agentReads ||= new Set(); owner.agentReads.add(record.agentId); }
        return { ...result, agentId: record.agentId };
      }
    }
    ...
    if (['send_prompt', 'terminal_interact', 'answer_question', 'permission'].includes(action.kind) && agentServices &&
        (typeof agentServices.enabled === 'function' ? agentServices.enabled() : agentServices.enabled)) {
      await requireFreshSessions(); check();
      if (agentServices.error()) throw new Error(agentServices.error());
      const agent = agentServices.directory.forSurface(action.targetId || action.target?.id);
      if (agent?.identity.state === 'ambiguous') throw new Error('This native conversation has ambiguous or competing agent owners. No input was sent.');
    }
