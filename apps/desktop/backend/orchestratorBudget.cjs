...
function fitMessages({ messages, tools = [], contextLength, outputTokens = 1200, maxBytes } = {}) {
  ...
  for (const name of require('../shared/orchestratorAgentTools.cjs').READS) readKinds.add(name);
  ...
  for (let i = 0; i < result.length && size() + 300 > budget;) {
    ...
    if (readonly) {
      try { readonly = calls.every(call => {
        const args = JSON.parse(call.function.arguments), name = call.function?.name;
        return readKinds.has(name === 'workspace' ? args.kind : name) && (name === 'workspace' || args.kind === undefined || args.kind === name);
