...
for (const priorCompleted of [false, true]) test(`written input with live idle agent${priorCompleted ? ' and prior completed turn' : ''} cannot become running in finish or status follow-up`, async t => {
  ...
  const sent = await f.app.send({ text: 'Send the bubble-label task to Codex.', origin: 'text' });
  assert.equal(sent.ok, true, JSON.stringify(sent));
  assert.match(sent.text, /Input was sent.*haven't confirmed that the task started/);
...
for (const sameBatch of [false, true]) for (const delivery of [
  ...
  assert.match(sent.text, delivery.status === 'written' ? /haven't confirmed that the task started/ : /couldn't confirm|could not be verified|Transport unavailable/);
});

test('new attributed turn allows running wording in finish and status responses', async t => {
  const f = await fixture(t, { started: true, priorCompleted: true });
  const sent = await f.app.send({ text: 'Send the bubble-label task to Codex.', origin: 'text' });
  assert.equal(sent.ok, true, JSON.stringify(sent));
  assert.match(sent.text, /task is running.*agent result is still pending/);
  f.setMode('status');
