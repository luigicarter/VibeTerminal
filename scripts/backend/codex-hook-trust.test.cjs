...
test('POSIX normalized hook identities have stable known hashes and exact source keys', () => {
  ...
  assert.equal(codexLifecycleConfigOverrides('/usr/bin/node', '/tmp/lina/observer.cjs', false).length, 6);
});
...
test('cached observer is repaired before trust and later tampering or absence withholds session trust', async t => {
  ...
  assert.equal(trustEntries(a.env.VIBE_TERMINAL_CODEX_HOOK_TRUST_OVERRIDE).length, 6);
  ...
  assert.equal(trustEntries((await next.prepareSession('repaired', { provider: 'codex' })).env.VIBE_TERMINAL_CODEX_HOOK_TRUST_OVERRIDE).length, 6);
