...
async function fixture(t){const temporaryParent=fs.realpathSync(os.tmpdir()),root=fs.mkdtempSync(path.join(temporaryParent,'vibe-file-navigation-')),steps=[],file=path.join(root,'source.txt');fs.writeFileSync(file,'x'.repeat(20000));let modelError;
