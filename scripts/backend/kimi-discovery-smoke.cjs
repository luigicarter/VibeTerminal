...
const root = path.join(
  rootDir,
  ".tmp",
  `kimi-discovery-smoke-${Date.now()}-${process.pid}`
);
...
// The kimi discovery functions read KIMI_CODE_HOME at call time, so point it at
// our fixture before requiring the host module.
...
// sessionDir points at a <home>/sessions/<bucket>/<id>-style directory holding
// state.json ({ title, lastPrompt, createdAt, updatedAt } — ISO strings).
function writeSession(id, workDir, state) {
  const sessionDir = path.join(kimiHome, "sessions", `wd_fixture_${id}`, id);
...
try {
  ...
  let result = find();
  assert(
    result && result.id === "alpha" && result.provider === "kimi",
  ...
  // Sessions from another folder never leak into this one (the kimi index is
  ...
  fs.appendFileSync(
    ...
    `${JSON.stringify({
      sessionId: "ghost",
      sessionDir: path.join(kimiHome, "sessions", "wd_fixture_ghost", "ghost"),
      workDir: cwd
    })}\n`
  );
  ...
  // listKimiThreads backs the resume picker: every session for the folder,
  // newest first, foreign folders excluded.
  const listed = listKimiThreads(cwd, 0, []);
  assert(
  ...
  assert(
    listed.threads[0].id === "epsilon-long" &&
      listed.threads.every((thread) => thread.provider === "kimi"),
    "list should be newest-first kimi refs"
  );
  assert(
    listKimiThreads(cwd, 0, ["epsilon-long", "gamma", "beta", "alpha"]).threads
      .length === 0,
    "list should honor excludeIds"
  );

  // confirmKimiThread underpins self-healing resume: only `kimi --session` an
  // id that still exists; otherwise the launcher must start fresh.
  const confirmed = confirmKimiThread(cwd, "alpha");
  assert(
  ...
  console.log("Kimi discovery smoke passed");
} finally {
