...
// Shape taken from a live wake cycle: pre-roll frame RMS was
// [0 x8, 0.0267, 0.15116, 0.07191, 0.13712, 0.07074, 0.01346, 0.00126, 0 x4] and the
// speaker had not started the command yet. Pre-roll silence must not spend the
...
const wakePreRoll = () => [...Array(8).fill(0).map(silent), ...Array(6).fill(0).map(voiced), ...Array(5).fill(0).map(silent)];

test('wake pre-roll silence does not consume the speaker trailing-silence budget', () => {
  const recording = createRecording({ preRoll: wakePreRoll() });
  // The speaker pauses for 1.2 s after the wake word before starting their command.
  for (let i = 0; i < 12; i++) assert.equal(recording.push(silent()), 'recording', `closed early at live frame ${i + 1}`);
});

test('a command spoken after the wake pause is captured whole with its pre-roll', () => {
  const recording = createRecording({ preRoll: wakePreRoll() });
...
test('a wake with no command still gives up only after the initial silence grace', () => {
  const recording = createRecording({ preRoll: wakePreRoll() });
  let state;
