...
app.whenReady().then(async () => {
  ...
  fs.writeFileSync(${JSON.stringify(path.join(brand,'lina-logo-1024.png'))}, vector.toPNG());
  const adaptive = '<!doctype html><html><style>html,body{margin:0;width:1024px;height:1024px;background:transparent}#safe{position:absolute;left:17%;top:17%;width:66%;height:66%}svg{display:block;width:100%;height:100%}</style><body><div id="safe">' + ${JSON.stringify(svg)} + '</div></body></html>';
  await win.loadURL('data:text/html;base64,' + Buffer.from(adaptive).toString('base64'));
  await win.webContents.executeJavaScript('new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))');
  await new Promise(resolve => setTimeout(resolve, 250));
  const foreground = await win.webContents.capturePage();
  fs.writeFileSync(${JSON.stringify(path.join(brand,'lina-logo-adaptive-1024.png'))}, foreground.toPNG());
  ...
  console.log('Exported Lina logo: SVG, 512px PNG, 1024px PNG, 1024px adaptive-icon PNG, and 7-resolution Windows ICO.');
  app.quit();
