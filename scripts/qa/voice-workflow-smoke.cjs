...
async function run(name, audio, commandStart, commandEnd) {
  ...
  const controller = createVoiceController({ modelPath: path.join(root, 'vendor/voice'), getKey: () => 'offline-key', getSettings: () => ({ handsFreeEnabled: true }),
    ...
    fetch: async (url, init) => { assert.ok(url.endsWith('/transcriptions')); uploads.push({ atMs: position / 16, wavMs: (Buffer.from(JSON.parse(init.body).input_audio.data, 'base64').length - 44) / 32 }); return { ok: true, json: async () => ({ text: commandStart != null && position >= commandStart ? 'Hey Vibe, open the project and run the tests' : 'Hey Vibe' }) }; }
  });
  try {
