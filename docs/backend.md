...
Codex `UserPromptSubmit` observer produce it, and so does Qwen's settings.json `UserPromptSubmit`
hook. Codex keeps an Enter/watchdog compatibility fallback when
...
- **qwen** - Qwen Code CLI (global npm `@qwen-code/qwen-code`, launched as `qwen` from PATH) reads
  hooks from `settings.json` in its config home (`$QWEN_HOME` or `~/.qwen`; no per-invocation flag),
  so `ensureQwenHooks()` (called from `terminal:create` when the launch command matches `qwen`)
  idempotently **merges** hook groups into the user's settings.json — the kimi model, JSON instead of
  TOML. Our entries are recognised by `name: "vibeterminal-notify"`; user settings keys and the user's
  own hooks (which stay ahead of ours on shared events) are preserved; `cleanupQwenHooks` strips the
  entries on shutdown (a settings file we created is removed outright, a malformed one is never
  clobbered). Event set is claude-parity plus qwen's native delegation bracket (verified accepted by
  qwen-code 0.21.6): `UserPromptSubmit`->`agent.running`, `PreToolUse`/`PostToolUse`->
  `agent.running tool`, `PermissionRequest`->`agent.waiting approval`, `Stop`->`agent.completed`,
  `StopFailure`->`agent.failed`, `SubagentStart`/`SubagentStop`->the delegation bracket (fired on the
  parent session, the kimi-custom shape). **Windows gotcha (verified):** qwen runs command hooks
  through its shell executor, whose default (ComSpec) mangles a nested `powershell -File ...` launch —
  the PowerShell banner leaks into the TUI as hook output and the program never runs — so each entry
  pins `shell: "powershell"` and invokes the notify program with the call operator
  (`& '<notify.ps1>' <type> [detail]`, forward slashes). Hook `timeout` is milliseconds (kimi's is
  seconds). Hooks fire in the interactive TUI (pane mode); headless `-p` runs do not fire them. The
  env guard keeps the merged hooks inert for `qwen` runs outside vibeTerminal panes.
