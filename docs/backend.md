...
- **kimi-custom** - The vendored custom fork (ribbon label "Kimi + CC") rides the same
  ...
  (`# vibeterminal-kimi-custom-notify`) and its own app-owned home
  (`<userData>/kimi-custom-home`; `VIBE_KIMI_CUSTOM_HOME` overrides). `terminal:create`
  matches `kimi-custom` first (the stock `kimi` gate's regex excludes it via `(?!-)`),
  inserts `vendor/kimi-custom/bin` into the session PATH *after* the shim dir (so the
  shim wrapper keeps the process-exit fallback) and into `VIBE_TERMINAL_ORIGINAL_PATH`
  (so the shim runner resolves the real launcher), injects `KIMI_CODE_HOME` pointing at
  the app-owned home, then `ensureKimiCustomHooks()` merges the identical claude-parity
  `[[hooks]]` set there; `cleanupKimiCustomHooks` strips them on shutdown. The launcher
  (`bin/kimi-custom.cmd` / `bin/kimi-custom`) supplies the fork's model env defaults
  (kimi-k3 on Moonshot's Anthropic-compatible endpoint, 1M context) and reads the API
  key from a gitignored `vendor/kimi-custom/api.txt` or an existing
  `KIMI_MODEL_API_KEY`; the user's stock `~/.kimi-code` is never read or written.
