...
- `backend/cliProbe.cjs` - Launch-time scan for which agent CLIs exist on the user's PATH, so the launcher can dim the ones that are missing. One `readdir` per PATH directory plus in-memory matching (O(directories), ~2-6ms for all seven commands on a 44-entry PATH) rather than stat-per-candidate (~76ms) or `where.exe` per command (~169ms); a missing command is the expensive case for the other two approaches. Per-directory and total timeouts keep an unreachable network share on PATH from stalling startup. Reports presence only - not that the CLI runs or is authenticated - so the renderer treats a miss as a hint and never blocks the launch.
