...
- `scripts/backend/fusion-appserver-smoke.cjs` - Boots Codex `app-server` over stdio, validates the initialize handshake, and verifies a Fusion-shaped thread can start with native Goals and nested multi-agent features explicitly disabled; `--require-embedded` requires `vendor/codex-bin` and is used by release checks.
...
- `npm run smoke:provider-startup` (`scripts/qa/provider-startup-probe.cjs`) - Opens one pane of every launchable PTY kind the way the app does (same shim directory, same launch command, same ConPTY host) in a scratch git repo under `.tmp/provider-startup-probe`, decodes it with `backend/terminalObservation.cjs`, polls `assessNativePromptReadiness` the way `waitForNativePromptReady` does, answers only a folder-trust screen whose affirmative option is already highlighted, then types one short line and confirms the pane took it. Prints a row per kind (verdict, seconds to ready, typed, accepted) and exits non-zero if a kind with a verified recognizer never reaches its composer. Kinds needing an account this machine lacks report `skipped` with the reason. It costs one tiny model turn per kind that reaches its composer; `--no-type` checks readiness without spending anything, `--only <kind,kind>` narrows the run, and `--save <dir>` re-records the fixtures under `scripts/backend/fixtures/provider-startup-screens`.
- `npm run smoke:orchestrator:ladder -- --tiers 1-4 --budget 2` (`scripts/qa/orchestrator-completion-ladder.cjs`) - Runs the graded utterance corpus (`scripts/backend/fixtures/orchestrator-completion-ladder.json`) through the REAL app on a scratch profile: real Codex/Claude panes whose model calls go to a local stub, the real Brain from the installed key (copied encrypted, never printed). Grades each turn on completion from the app's own records and writes `apps/desktop/.tmp/orchestrator-ladder/<run>/scoreboard.md` beside every model call. `--models a,b` compares Brains, `--local-brain http://127.0.0.1:11434/v1 --models qwen3.5:2b` measures a local one, `--scenario T2.1 --visible` runs one scenario on screen. Only the Brain costs money. See `docs/orchestrator-completion-ladder-2026-09-14.md`.
