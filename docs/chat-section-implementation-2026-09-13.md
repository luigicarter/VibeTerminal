...
- Selected-project/Multi filtering, an All chats toggle, local metadata search, incremental row display, New chat provider picker, sidebar title overrides, archive/unarchive, and native history refresh in known folders.
...
- A chat's native identity and launch settings survive pane closure and project removal. Closed project histories remain reachable through All chats. Plain shell panes are excluded from Chats.
